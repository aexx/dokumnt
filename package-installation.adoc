...
$ platz () { df -m / ;ls /var/cache/apt/archives/|wc -l ; } ;platz >/dev/shm/platz1 ;sudo apt clean;sudo apt autoremove ;platz >/dev/shm/platz2 ; sdiff /dev/shm/platz1 /dev/shm/platz2 
$ platz () { df -m / ;df -h / ;ls /var/cache/apt/archives/|wc -l ; } ;platz >/dev/shm/platz1 ;sudo nala clean;sudo nala autoremove ;platz >/dev/shm/platz2 ; sdiff /dev/shm/platz1 /dev/shm/platz2 
----
...
.Fehler|error waehrend|during update
----
mintUpdate: chrome repository 
# W:Updating from such a repository can't be done securely, and is therefore disabled by default., W:See apt-secure(8) manpage for repository creation and user configuration details., E:The repository 'http://dl.google.com/linux/chrome/deb stable Release' no longer has a Release file.
## NEUER|NEW linux_signing_key
$ wget -q -O - https://dl.google.com/linux/linux_signing_key.pub | sudo tee /etc/apt/trusted.gpg.d/google.asc >/dev/null
----

.SpiderOak Apt Repository
----
...
.Tages Zaehler GUI Update Manager (spideroak) zaehlt nicht hoch    
----
$ sudo apt-get update 
...
[...] 
## Browser: => http://apt.spideroak.com  
=> wget http://apt.spideroak.com/spideroak-apt-pubkey.asc -O new_key  
=> sudo apt-key add new_key  =>  sudo apt-get update
----
...
----
watch -d -n 4 'df -am ; ps aux |grep rpm |grep -v grep' # Update Beobachten/watch 
